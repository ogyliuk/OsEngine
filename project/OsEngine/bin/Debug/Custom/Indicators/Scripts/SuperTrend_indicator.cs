...
// https://ru.tradingview.com/script/AhhUQJGV-3-SuperTrends-ATR-SL-3-EMAs/
...
public class SuperTrend_indicator : Aindicator
{
    ...
    public override void OnStateChange(IndicatorState state)
    {
        if (state == IndicatorState.Configure)
        {
            ...
            _wicks = CreateParameterBool("Use tails", true);

            _seriesLower = CreateSeries("Lower channel", Color.Aqua,
                IndicatorChartPaintType.Line, false);
            _seriesLower.CanReBuildHistoricalValues = false;

            _seriesUpper = CreateSeries("Upper channel", Color.OrangeRed,
                IndicatorChartPaintType.Line, false);
            _seriesUpper.CanReBuildHistoricalValues = false;

            _seriesCenter = CreateSeries("Center Line ", Color.Gold,
